...
const setupQueryEditors = () => {
  ...
  qe = newState.queryEditors.find(
    (e: SqlLabState['queryEditors'][number]) => e.id === 'abcd',
  );
};
...
const setupClearInactiveQueries = () => {
  query = {
    id: 'abcd',
    changed_on: Date.now(),
    startDttm: Date.now(),
